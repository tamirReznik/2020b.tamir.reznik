...
public class ElementTests {
	...
	public void test_Create_New_Element_And_Check_If_DB_Contatins_Same_ElementID() throws Exception{
		ElementBoundary eb = new ElementBoundary(new ElementIdBoundary(), TypeEnum.actionType, "moshe", true, new Date(), new Location(), null,null);
		ElementIdBoundary postedElementId = this.restTemplate.postForObject(this.url + "/elements/aaa/bbb",eb , ElementBoundary.class).getElementId();
		
		ElementBoundary[] allElements = this.restTemplate.getForObject(this.url + "/elements/aaa/bbb", ElementBoundary[].class);
		boolean exist = false;
	...
	public void test_Create_Two_Elements_Get_Specific_One_And_See_If_ID_Matches() throws Exception {
		ElementBoundary eb1 = new ElementBoundary(new ElementIdBoundary(), TypeEnum.actionType, "moshe", true, new Date(), new Location(), null,null);
		ElementBoundary eb2 = new ElementBoundary(new ElementIdBoundary(), TypeEnum.actionType, "david", true, new Date(), new Location(), null,null);
		...
		ElementBoundary ebCheck = this.restTemplate.getForObject(this.url + "/elements/aaa/bbb/{elementDomain}/{elementId}", ElementBoundary.class, eb1.getElementId().getDomain(),eb1.getElementId().getId());
	...
	public void test_Update_Element_And_Check_If_Update_Succeeded() throws Exception{
		...
		this.restTemplate.put(this.url + "/elements/aaa/bbb/{elementDomain}/{elementId}", eb, postedElementId.getDomain(),postedElementId.getId());
